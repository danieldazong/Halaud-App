...
**Temporary exception — narration and library state while previewing in Expo Go:** `expo-sqlite` is a native module and cannot run in Expo Go (see Native Modules Rule). While the app is being previewed in Expo Go rather than a development build, two stores persist via Zustand's `persist` middleware backed by `@react-native-async-storage/async-storage` instead of SQLite — AsyncStorage is one of the few native modules Expo Go bundles internally, so it works there:

- `store/narration.ts` — `languageCode`, `voiceId`, `recentLanguageCodes`.
- `store/libraryStore.ts` — the imported `documents` list (id, title, uri, fileSize, importedAt, progress). Note: document `uri` values point into the app's cache directory (`expo-document-picker`'s `copyToCacheDirectory: true`), not the original content-provider URI, so they remain readable across a relaunch — but the OS can still clear app cache under storage pressure, which SQLite-backed storage in `documents`/`audio_cache` will handle properly once that migration happens.

Swap both back to their SQLite-backed tables (`preferences`, `documents`) once the app moves to a development build full-time; `db/database.ts` already has a working SQLite implementation of the narration interface from an earlier pass, so that swap is a re-swap, not new work.
